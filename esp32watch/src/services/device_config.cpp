...
namespace {
constexpr const char *kPrefsNs = "watch_cfg";
...
{
    return g_prefs.begin(kPrefsNs, read_only);
}
...
static void load_from_preferences(void)
{
    ...
    if (loaded_from_slots) {
        ...
        update_derived_flags();
        (void)persist_snapshot(g_snapshot, g_wifi_password, g_api_token);
    }
...
extern "C" bool device_config_authenticate_token(const char *token)
{
    ...
    return token != nullptr && strcmp(token, g_api_token) == 0;
}
